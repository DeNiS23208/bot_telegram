    ...
    if payment_id:
        try:
            ...
            if tg_user_id:
                ...
                print(f"🔄 Обработка возврата из ЮKassa: payment_id={payment_id}, status={status}, tg_user_id={tg_user_id}")
                
                # Обновляем статус в БД
                await update_payment_status_async(payment_id, status)
                
                # Если платеж pending или canceled - МГНОВЕННО отправляем уведомление о выходе из оплаты
                if status in ("pending", "canceled"):
                    # ВСЕГДА отправляем уведомление, независимо от подписки
                    ...
                    # Если ссылка не найдена в БД, получаем её из ЮKassa API
                    if not payment_url:
                        try:
                            from payments import get_payment_url
                            payment_url = get_payment_url(payment_id)
                        except Exception as e:
                            print(f"⚠️ Ошибка получения ссылки на оплату из API: {e}")
                    ...
                    # МГНОВЕННО отправляем уведомление пользователю (не ждем проверки подписки)
                    try:
                        await bot.send_message(tg_user_id, message_text)
                        print(f"✅ МГНОВЕННО отправлено уведомление о выходе из оплаты пользователю {tg_user_id}")
                    except Exception as e:
    ...
    if event == "payment.canceled":
        ...
        try:
            ...
            if tg_user_id:
                ...
                # ВСЕГДА отправляем уведомление при выходе из оплаты, независимо от наличия активной подписки
                # Потому что пользователь может выходить из оплаты несколько раз по одной и той же ссылке
                try:
                    await bot.send_message(tg_user_id, message_text)
                    print(f"✅ Отправлено уведомление об отмене платежа пользователю {tg_user_id} (выход из оплаты)")
                except Exception as e:
