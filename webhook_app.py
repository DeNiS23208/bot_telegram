        ...
        try:
            ...
            if tg_user_id:
                ...
                try:
                    ...
                    if cancellation_details_final:
                        ...
                        if any(keyword in reason for keyword in ['insufficient', 'funds', 'недостаточно', 'money', 'balance']):
                            ...
                            # Если party содержит user - значит пользователь отменил (не отправляем уведомление)
                            if 'user' in party:
                                cancellation_reason = "отменен пользователем (выход из формы)"
                                message_text = None  # Не отправляем уведомление, если пользователь сам вышел
                            else:
                            ...
                            # По умолчанию считаем, что пользователь отменил (не отправляем уведомление)
                            cancellation_reason = "отменен пользователем (выход из формы)"
                            message_text = None  # Не отправляем уведомление, если пользователь сам вышел
                    else:
                        # Если нет деталей, по умолчанию считаем что пользователь вышел из формы (не отправляем уведомление)
                        cancellation_reason = "отменен пользователем (выход из формы)"
                        message_text = None  # Не отправляем уведомление, если пользователь сам вышел
                    
                    # Если message_text пустой или None, не отправляем уведомление (пользователь сам вышел)
                    if not message_text:
                        cancellation_reason = "отменен пользователем (выход из формы)"
                        message_text = None  # Не отправляем уведомление
                        
                except Exception as e:
                    logger.warning(f"⚠️ Ошибка при определении причины отмены: {e}")
                    import traceback
                    logger.debug(traceback.format_exc())
                    # В случае ошибки не отправляем уведомление (скорее всего пользователь сам вышел)
                    cancellation_reason = "отменен пользователем (выход из формы)"
                    message_text = None  # Не отправляем уведомление
                ...
                if has_active:
                    ...
                    # Уведомляем пользователя только если есть текст сообщения (не пустое и не None)
                    try:
                    ...
                    # Если message_text пустое или None - не отправляем уведомление (пользователь сам вышел)
                    logger.info(f"ℹ️ Платеж {payment_id} отменен пользователем (выход из формы) - уведомление не отправлено")
            else:
