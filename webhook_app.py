                ...
                try:
                    ...
                    if cancellation_details:
                        ...
                        if isinstance(cancellation_details, dict):
                            reason = str(cancellation_details.get('reason', '')).lower()
                            party = str(cancellation_details.get('party', '')).lower()
                        else:
                            reason = str(getattr(cancellation_details, 'reason', '')).lower()
                            party = str(getattr(cancellation_details, 'party', '')).lower()
                        
                        print(f"🔍 Причина отмены: reason={reason}, party={party}, details={cancellation_details}")
                        ...
                        if any(keyword in reason for keyword in ['insufficient', 'funds', 'недостаточно', 'money', 'balance']):
                            ...
                            cancellation_reason = "другая причина"
                            message_text = (
                                ...
                                "Проверьте баланс карты и попробуйте еще раз пройти по ссылке на оплату.\n"
                                "Для оплаты нажмите кнопку 💳 Оплатить доступ."
                            )
                    else:
                        # Если нет деталей, отправляем общее сообщение
                        cancellation_reason = "нет деталей"
                        message_text = (
                            "❌ Платёж не был завершён\n\n"
                            "Проверьте баланс карты и попробуйте еще раз пройти по ссылке на оплату.\n\n"
                            "Для оплаты нажмите кнопку 💳 Оплатить доступ и перейдите по новой ссылке."
