    ...
    if event == "payment.canceled":
        ...
        try:
            ...
            if tg_user_id:
                ...
                cancellation_reason = "отменен пользователем (выход из оплаты)"
                ...
                if has_active:
                    ...
                    # Уведомляем пользователя только если нет активной подписки
                    # Если message_text пустой, формируем стандартное сообщение
                    if not message_text:
                        # Получаем ссылку на оплату из БД
                        from db import get_payment_url_from_db
                        payment_url = await get_payment_url_from_db(payment_id)
                        ...
                        if payment_url:
                            ...
                            try:
                                ...
                                print(f"⚠️ Ошибка получения ссылки на оплату: {e}")
                                message_text = (
                                    "❌ Оплата не произведена\n\n"
                                    "Вы вышли из формы оплаты без завершения платежа.\n\n"
                                    "Для оплаты нажмите кнопку 💳 Оплатить доступ на 30 дней и перейдите по новой ссылке."
