    ...
    try:
        ...
        if refreshed_status == "succeeded":
            ...
            if attempts_after_failure >= ban_threshold:
                ...
                if insufficient_funds:
                    await safe_send_message(
                        ...
                        text=(
                            ...
                            "Попытка 1 из 3 не удалась. Продолжаем попытки автопродления.\n\n"
                            "Для возобновления доступа используйте кнопку 💳 Получить доступ."
                        ),
                        parse_mode="HTML",
                        reply_markup=menu
                    )
                    logger.info(f"📧 Отправлено уведомление о недостаточности средств пользователю {telegram_id} (попытка {attempts_after_failure} из {max_attempts})")
                else:
                    await safe_send_message(
                        bot=bot,
                        chat_id=telegram_id,
                    ...
                    logger.info(f"📧 Отправлено уведомление о неудачном автопродлении пользователю {telegram_id} (попытка {attempts_after_failure} из {max_attempts})")
                ...
                # Отправляем сообщение об истечении доступа
                await safe_send_message(
                    bot=bot,
                    chat_id=telegram_id,
                    text=(
                        ...
                        "Продолжаем попытки автопродления.\n"
                        "Для возобновления доступа нажмите кнопку 💳 Получить доступ."
                    ),
                    parse_mode="HTML",
                    reply_markup=menu
                )
                logger.info(f"📧 Отправлено уведомление об истечении доступа пользователю {telegram_id} (попытка {attempts_after_failure} из {max_attempts})")
                ...
                if insufficient_funds:
                    await safe_send_message(
                        ...
                        text=(
                            "⚠️ <b>У вас недостаточно средств</b>\n\n"
                            "На вашей карте недостаточно средств для автопродления подписки.\n\n"
                            f"Попытка {attempts_after_failure} из {max_attempts} не удалась. Продолжаем попытки автопродления.\n\n"
                            "Для возобновления доступа используйте кнопку 💳 Получить доступ."
                        ),
                        parse_mode="HTML",
                        reply_markup=menu
