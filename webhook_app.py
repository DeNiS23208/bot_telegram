...
async def get_main_menu_for_user(telegram_id: int) -> ReplyKeyboardMarkup:
    ...
    if bonus_week_active:
        ...
        if auto_renewal_in_progress:
            # Идут попытки автопродления - сохраняем текущее меню пользователя
            if show_manage_button:
                # У пользователя была активная подписка с автопродлением - показываем "Управление доступом"
                ...
                # У пользователя не было активной подписки с автопродлением - показываем бонусное меню
...
async def attempt_auto_renewal(telegram_id: int, saved_payment_method_id: str, auto_amount: str, auto_duration: float, attempt_number: int) -> bool:
    ...
    try:
        ...
        if refreshed_status == "succeeded":
            ...
            # КРИТИЧЕСКИ ВАЖНО: На 3 попытке сразу отправляем сообщение об истечении доступа и обновляем меню
            if attempts_after_failure >= 3:
                # Все 3 попытки неудачны - отправляем сообщение об истечении доступа с продакшн меню
                ...
                # Отправляем сообщение о недостаточности средств или ошибке
                ...
                # Сразу отправляем сообщение об истечении доступа с обновленным меню
                await safe_send_message(
                    bot=bot,
                    chat_id=telegram_id,
                    text=(
                ...
                # Еще не все попытки выполнены - отправляем уведомление БЕЗ меню
                if insufficient_funds:
                    await safe_send_message(
                        bot=bot,
                        chat_id=telegram_id,
                        ...
                        parse_mode="HTML"
                        # НЕ отправляем reply_markup - меню не меняется
                    )
                else:
                    await safe_send_message(
                        bot=bot,
