...
async def get_main_menu_for_user(telegram_id: int) -> ReplyKeyboardMarkup:
    ...
    if bonus_week_active:
        ...
        # показываем ТОЧНО ТАКОЕ ЖЕ меню, как в бонусной подписке (с кнопкой "Бонус в честь запуска канала")
        # Меню НЕ должно меняться до завершения всех 3 попыток
        if auto_renewal_in_progress:
            # Идут попытки автопродления - показываем бонусное меню, как было до окончания бонусной недели
