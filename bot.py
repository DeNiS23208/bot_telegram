...
BTN_CANCEL_SUB = "❌ Отменить подписку и отключить автопродление"  # Показывается в меню управления если автопродление включено
...
# Обработчик для кнопки "Отменить подписку и отключить автопродление"
@dp.message(lambda m: (m.text or "").strip() == BTN_CANCEL_SUB)
async def cancel_subscription(message: Message):
    """Обработчик кнопки отмены подписки - отключает автопродление и удаляет способ оплаты"""
    user_id = message.from_user.id
    await send_typing_action(message.chat.id)
    ...
    await message.answer(
        ...
        "💳 Данные карты удалены.\n\n"
        f"📅 <b>Подписка действует до:</b> {expires_str}\n\n"
        f"💰 <b>Стоимость подписки:</b> {PAYMENT_AMOUNT_RUB} рубль\n\n"
        "💡 После окончания срока подписка не будет продлеваться автоматически.\n\n"
        "🔄 Для возобновления автопродления необходимо оплатить подписку заново.",
        parse_mode="HTML",
