...
async def auto_renewal_toggle(message: Message):
    ...
    # Формируем сообщение о текущем статусе (временно для скриншотов)
    # ВРЕМЕННО: показываем всегда как включено с привязанной картой
    status_text = "✅ Автопродление подписки включено\n"
    status_text += "✅ Карта привязана\n\n"
    status_text += "⚠️ Для отключения автопродления необходимо нажать на \"Отключить автопродление\"\n"
    status_text += "⚠️ Для отключения автопродления и отвязки карты необходимо нажать на \"Отвязать карту и отключить автопродление\""
...
async def unlink_and_disable_callback(callback: CallbackQuery):
    ...
    if saved_method:
        ...
        # Если карты нет, просто отключаем автопродление (временно для теста)
        await set_auto_renewal(user_id, False)
        await callback.answer("Карта отвязана и автопродление отключено")
        await callback.message.edit_text(
            "Карта ****-1234 💳 отвязана от сервиса\n"
            "❌ Автопродление подписки отключено\n\n"
            "Ваша подписка не будет автоматически продлеваться.",
            reply_markup=None
        )
