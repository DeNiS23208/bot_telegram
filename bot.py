...
# Будет заменяться динамически с payment_id при создании платежа
WEBHOOK_BASE_URL = os.getenv("WEBHOOK_BASE_URL", "https://your-server.com")  # Нужно указать реальный URL вашего сервера
RETURN_URL_TEMPLATE = f"{WEBHOOK_BASE_URL}/payment/return"
...
async def cmd_start(message: Message):
    ...
    # Формат: /start payment_return или /start payment_exit_{payment_id}
    if message.text and "payment_return" in message.text:
        ...
        try:
            # Проверяем последний pending платеж, созданный в последние 2 минуты (для мгновенной реакции)
            active_payment = await get_active_pending_payment(message.from_user.id, minutes=2)
            
            if active_payment:
                payment_id, created_at, payment_url = active_payment
                
                # МГНОВЕННО отправляем уведомление, не проверяя статус (это сделает webhook)
