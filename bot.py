...
# URL для возврата из ЮKassa - ведет в бота с параметром для определения возврата из оплаты
...
async def cmd_start(message: Message):
    ...
    if message.text and "payment_return" in message.text:
        print(f"🔄 Получен возврат из оплаты от пользователя {message.from_user.id}")
        # Пользователь вернулся из ЮKassa - МГНОВЕННО проверяем последний pending платеж
        try:
            # Проверяем последний pending платеж БЕЗ ограничения по времени
            # Это гарантирует мгновенную отправку уведомления при любом возврате
            ...
            if latest_payment:
                ...
                # МГНОВЕННО отправляем уведомление, не проверяя время (это сделает webhook)
                ...
                print(f"📤 Отправляем уведомление пользователю {message.from_user.id}")
                await message.answer(exit_message, reply_markup=main_menu())
                print(f"✅ Уведомление отправлено пользователю {message.from_user.id}")
                return
            else:
                print(f"⚠️ Не найден pending платеж для пользователя {message.from_user.id}")
