...
# ID аккаунта поддержки (не проверяется и не банится)
SUPPORT_ACCOUNT_ID = 8429417659  # otd_zabota
...
async def cmd_start(message: Message):
    ...
    if is_bonus_week_active():
        ...
        welcome_text = (
            ...
            f"• Доступ к закрытому каналу всего за <b>{BONUS_WEEK_PRICE_RUB} рубль</b>\n"
...
async def about(message: Message):
    ...
    await message.answer(
        ...
        "Обратитесь к нашему менеджеру поддержки:\n"
        "👤 <b>@otd_zabota</b>",
        parse_mode="HTML",
...
async def bonus_week_info(message: Message):
    ...
    bonus_text = (
        ...
        f"Нажмите кнопку ниже, чтобы получить доступ за {BONUS_WEEK_PRICE_RUB} рубль 👇"
    )
...
async def manage_subscription(message: Message):
    ...
    # КРИТИЧЕСКИ ВАЖНО: Проверяем, активна ли подписка
    ...
    if not has_active_subscription and auto_renewal_in_progress and not bonus_week_active:
        # Идут попытки автопродления в продакшн режиме - показываем информацию о попытках
        # КРИТИЧЕСКИ ВАЖНО: Это продакшн режим, не бонусная неделя!
        
        # Создаем клавиатуру с кнопками управления автопродлением
        keyboard_buttons = []
        if auto_renewal_enabled:
        ...
        await message.answer(
            ...
            f"⚠️ <b>Идут попытки автопродления</b>\n"
            f"Попытка: {attempts}/3\n\n"
            f"Автоматическое продление доступа на полную стоимость.\n"
            f"Стоимость: <b>{get_production_subscription_price()} рублей</b>\n"
            f"Срок доступа: <b>{format_subscription_duration(get_production_subscription_duration())}</b>\n\n"
            f"Вы можете отключить автопродление в любой момент.",
            parse_mode="HTML",
    ...
    bonus_week_ended = now > bonus_week_end
    # КРИТИЧЕСКИ ВАЖНО: auto_renewal_in_progress = True если:
    # 1. Автопродление включено
    # 2. Есть попытки (attempts > 0) и их меньше 3
    # 3. Бонусная неделя закончилась
    auto_renewal_in_progress = auto_renewal_enabled and attempts > 0 and attempts < 3 and bonus_week_ended
    ...
    # КРИТИЧЕСКИ ВАЖНО: Если идут попытки автопродления, добавляем информацию о них
    attempts_info = ""
    if auto_renewal_in_progress:
        attempts_info = (
            f"\n⚠️ <b>Идут попытки автопродления</b>\n"
            f"Попытка: {attempts}/3\n"
            f"Автоматическое продление доступа выполняется.\n"
            f"Вы можете отключить автопродление в любой момент.\n\n"
        )
    ...
    # КРИТИЧЕСКИ ВАЖНО: Кнопки управления автопродлением должны показываться ВСЕГДА,
    # если автопродление включено, даже во время попыток автопродления
    if auto_renewal_enabled:
        keyboard_buttons.append([KeyboardButton(text=BTN_CANCEL_SUB)])
    else:
...
async def on_chat_member_update(update: ChatMemberUpdated):
    ...
    if update.new_chat_member.status == ChatMemberStatus.MEMBER:
        ...
        # ИСКЛЮЧЕНИЕ: аккаунт поддержки не проверяется и не банится
        if user_id == SUPPORT_ACCOUNT_ID:
            print(f"🛡️ Аккаунт поддержки {user_id} присоединился к каналу - пропуск проверки")
            return
