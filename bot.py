...
# Обработчик присоединения пользователей к каналу - проверяем владельца ссылки
@dp.chat_member()
async def on_chat_member_update(update: ChatMemberUpdated):
    """Отслеживает присоединение пользователей к каналу и проверяет, что присоединился владелец ссылки"""
    ...
    if update.new_chat_member.status == ChatMemberStatus.MEMBER:
        ...
        if invite_link:
            ...
            # Проверяем, что у владельца ссылки есть активная подписка
            if link_owner_id:
            ...
            if link_owner_id and link_owner_id == user_id:
                # Ссылка принадлежит этому пользователю - разрешаем доступ, НЕ отзываем ссылку
                # Ссылка останется активной, чтобы пользователь мог вернуться, если выйдет
                print(f"✅ Пользователь {user_id} (владелец ссылки) присоединился к каналу - доступ разрешен, ссылка остается активной")
            elif link_owner_id and link_owner_id != user_id:
                # Ссылка принадлежит другому пользователю - это попытка использования чужой ссылки
                # Баним нарушителя и отзываем ссылку для защиты
                try:
                    # Баним пользователя, который попытался использовать чужую ссылку
                    ...
                    print(f"🚫 Пользователь {user_id} попытался использовать чужую ссылку (owner: {link_owner_id}) - забанен, ссылка отозвана")
                except Exception as e:
                    print(f"⚠️ Ошибка при бане пользователя {user_id} или отзыве ссылки: {e}")
            elif not has_active_subscription:
                # Ссылка принадлежит пользователю, но подписка истекла - баним и отзываем ссылку
                try:
                    await bot.ban_chat_member(
                        chat_id=CHANNEL_ID,
                        user_id=user_id,
                    ...
                    print(f"🚫 Пользователь {user_id} попытался использовать ссылку с истекшей подпиской - забанен, ссылка отозвана")
                except Exception as e:
                    print(f"⚠️ Ошибка при бане пользователя {user_id} или отзыве ссылки: {e}")
